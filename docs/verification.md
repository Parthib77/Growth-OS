...
- `npm test` passed 12 files and 29 tests. Coverage includes configuration, CSV limits and parsing, duplicate review, lifecycle and booking transitions, formula-safe cells, customer import retry behavior, campaign template rendering, audience eligibility, recipient review, review response rules, results calculations, and CSV formula protection.
...
- The self-contained Playwright run passed all 14 active checks in desktop Chromium and the phone profile; 2 opt-in visual-capture checks were skipped. The suite covers axe checks, the database-backed demo, single-use password recovery, registration through booking completion and Results, the customer add/search/detail/history/consent-withdrawal workflow, the campaign draft/review/activation/handoff/outcome/audit workflow, reviews, settings, exports, legal drafts, and deletion confirmation.
